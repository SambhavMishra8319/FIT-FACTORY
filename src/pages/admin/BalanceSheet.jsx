...
export default function BalanceSheet() {
  ...
  // total business profit (real gym profit)
  ...
  notes: "",
      ...
      const trainerPaymentData = trainerSnap.docs.map((d) => {
        ...
        return {
          ...
          source: "trainerPayments",

          type: "expense",
          category: "Staff Salary",

          description: `Salary - ${t.trainerName}`,
  ...
  const trainerExpense = useMemo(() => {
    return filtered
      .filter((e) => e.type === "expense" && e.category === "Staff Salary")
  ...
  // const netBalance = totalIncome - totalExpense - trainerExpense;
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      toast.success(
        `${form.type === "income" ? "Income" : "Expense"} of ₹${Number(form.amount).toLocaleString()} added!`,
      );
      setShowForm(false);
      setForm({
        type: "expense",
  ...
  //     source === "trainerPayments" ? "trainerPayments" : "balance_sheet";
  //   // source: "payments";
  //   try {
  ...
  const handleDelete = async (id, source) => {
    ...
    // if (source === "payments") {
    //   toast.error("Membership payments can't be deleted from Balance Sheet");
    //   return;
    // }
    if (source === "payments") {
      toast.error("Delete this payment from Member Payments page.");
      return;
    }
  ...
  const gymProfit = totalIncome - totalExpense;
  const handleExport = () => {
    if (!filtered.length) {
  ...
  // Operating profit (without payroll)
  const operatingProfit = totalIncome - totalExpense;

  // Real gym profit (after trainer salary)
  // const gymProfit = operatingProfit - trainerExpense;
  const currentCategories =
    form.type === "expense" ? EXPENSE_CATEGORIES : INCOME_CATEGORIES;

  const profitMargin =
    totalIncome > 0 ? ((netBalance / totalIncome) * 100).toFixed(1) : 0;
  const operatingExpense = filtered
    .filter((e) => e.type === "expense" && e.category !== "Staff Salary")
    .reduce((s, e) => s + Number(e.amount || 0), 0);

  // const operatingProfit =
  //   totalIncome - operatingExpense;
    ...
    <div className="page-enter">
      ...
      <div className="page-body">
            ...
            <form onSubmit={handleAdd}>
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  placeholder={
                    form.type === "expense"
                      ? "e.g. New treadmill belt replacement"
                      : "e.g. Monthly membership payments"
                  }
              ...
              >
                {saving
                  ? "Saving…"
                  : `Add ${form.type === "expense" ? "Expense" : "Income"} Entry`}
              </button>
            </form>
          </div>
        )}
        ...
        >
          {[
            ...
            <button
              ...
              style={{
                ...
                borderRadius: "var(--r-sm)",
                background:
                  filterMode === f.key ? "var(--gold-dim)" : "var(--card2)",
          ...
          <span
            style={{ fontSize: 11, color: "var(--muted2)", marginLeft: "auto" }}
          >
        ...
        <div className="stats-grid mb-20" style={anim(0.1)}>
          ...
          <div className="stat-card s-red">
            ...
            <div className="stat-value c-red">
              ₹
              {trainerExpense >= 100000
                ? (trainerExpense / 100000).toFixed(1) + "L"
                : (trainerExpense / 1000).toFixed(1) + "k"}
            </div>
            <div className="stat-sub">Salary payout</div>
          </div>
          <div className={`stat-card ${netBalance >= 0 ? "s-gold" : "s-red"}`}>
            <div className="stat-label">Net Balance</div>
            <div
              className={`stat-value ${netBalance >= 0 ? "c-gold" : "c-red"}`}
            >
              {netBalance >= 0 ? "+" : ""}₹
              {Math.abs(netBalance) >= 100000
          ...
          <div className={`stat-card ${gymProfit >= 0 ? "s-gold" : "s-red"}`}>
            <div className="stat-label">Gym Profit</div>
            <div
              className={`stat-value ${gymProfit >= 0 ? "c-gold" : "c-red"}`}
            >
              {gymProfit >= 0 ? "+" : "-"}₹
              {Math.abs(gymProfit).toLocaleString()}
            </div>
            {/* <div className="stat-sub">After trainer payouts</div> */}
            <div className="stat-sub">Including trainer salary</div>
          </div>
              ...
              <ResponsiveContainer width="100%" height={160}>
                <BarChart
                  data={[
                    ...
                    {
                      name: "Net",
                      amount: Math.max(netBalance, 0),
                      fill: netBalance >= 0 ? "#f5c842" : "#e63329",
                    },
                  ]}
                ...
                >
                  ...
                  <Bar dataKey="amount" radius={[4, 4, 0, 0]}>
                    <Cell fill="#22c55e" />
                    <Cell fill="#e63329" />
                    <Cell fill={netBalance >= 0 ? "#f5c842" : "#e63329"} />
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
              ...
              {loading ? (
                ...
                <tr>
                  ...
                  >
                    ...
                    No entries for this period. Click{" "}
                    <strong>+ Add Entry</strong> to start tracking.
                  </td>
                </tr>
              ) : (
                filtered.map((e, i) => (
                  ...
                  >
                    ...
                    <td>
                      <span
                        className={`badge ${e.type === "income" ? "badge-green" : "badge-red"}`}
                      >
                    ...
                    <td>
                      <span
                        className={`badge ${e.paymentMode === "UPI" ? "badge-blue" : e.paymentMode === "Cash" ? "badge-gray" : "badge-gold"}`}
                      >
                        {e.paymentMode}
                      </span>
                    </td>
          ...
          >
            ...
            <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
              {[
                ...
                {
                  label: "Net Balance",
                  val: `${netBalance >= 0 ? "+" : ""}₹${netBalance.toLocaleString()}`,
                  color: netBalance >= 0 ? "var(--gold)" : "var(--red)",
                },
                {
                  label: "Trainer Expense",
                  val: `₹${trainerExpense.toLocaleString()}`,
                  color: "var(--red)",
                },
                {
                  label: "Entries",

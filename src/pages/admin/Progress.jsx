...
export default function Progress() {
  ...
  const last14 = Array.from({ length: 14 }, (_, i) => {
    ...
    return { date: dateStr, label: format(d, "d"), day: format(d, "EEE"), count };
  });
  ...
  return (
    <div className="page-enter">
      <div className="topbar"><div className="page-title">Progress Overview</div></div>
      <div className="page-body">
        ...
        <div className="card mb-20" style={anim(0.28)}>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "space-between" }}>
            {last14.map((d, i) => {
              ...
              return (
                <div key={i} style={{ textAlign: "center", flex: "1 0 auto", minWidth: 36 }}>
                  <div style={{
                  ...
                  <div style={{ fontSize: 9, color: "var(--muted2)" }}>{d.label}</div>
                  <div style={{ fontSize: 8, color: "var(--muted)" }}>{d.day}</div>
                </div>
              );
            })}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 12, fontSize: 11, color: "var(--muted2)" }}>
            <span>Less</span>
            {[0.1, 0.3, 0.5, 0.7, 0.9].map(v => (
              <div key={v} style={{ width: 14, height: 14, borderRadius: 3, background: `rgba(245,200,66,${v})` }} />
            ))}
            <span>More</span>
          </div>
        </div>
        ...
        <div style={anim(0.35)}>
          ...
          {loading ? (
            [1,2,3].map(i => <div key={i} className="skeleton mb-8" style={{ height: 52, borderRadius: "var(--r-sm)" }} />)
          ) : topMembers.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 20px", color: "var(--muted2)", fontSize: 13 }}>
            ...
            >
              <div style={{
                ...
                width: 26, color: i < 3 ? "var(--gold)" : "var(--muted2)",
              }}>
                {i + 1}
              </div>
              ...
              <div className="lb-info">
                ...
                <div style={{ fontSize: 11, color: "var(--muted2)" }}>{m.count} sessions in 30 days</div>
              </div>
              {/* Attendance bar */}
              <div style={{ flex: 1, maxWidth: 120 }}>
                ...
                <div style={{ fontSize: 10, color: "var(--muted2)", marginTop: 3, textAlign: "right" }}>
